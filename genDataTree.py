...
entities = os.listdir('data/active_entities')
entitylist = []
tempCount = 0
...
def make_package(
    ...
    children = package['rule']['children'][1]['children']
    entityCount = {}
    count = 0
    ...
    for entityKey in entityCount:
        if entityCount[entityKey] > count / 2:
...
for kind in train:
    with open('data/active_packages/%s' % kind + '.json', 'w', encoding='utf-8') as fout:
...
with open('sentence-simulator-master/TreeSum.json', 'w', encoding='utf-8') as fwrite:
    with open('TreeSum.json', 'w', encoding='utf-8') as fw:
        package = {
            'rule': {
                'type': "root",
                'children': [
        ...
        children = package['rule']['children']
        for name in os.listdir('data/active_packages'):
            with open("data/active_packages/" + name, "r") as fRead:
                test = json.load(fRead)
                children.append(test['rule']['children'][1])
        entity = package['entity']
        for name in os.listdir('data/active_entities'):
            with open("data/active_entities/" + name, "r") as fRead:
                list = []
                while 1:
                    line = fRead.readline()[0:-1]
                    if (len(line) > 0):
                        list.append(line)
                    if not line:
                        break
                ...
                fRead.close()
        fwrite.write(json.dumps(package))
        fw.write(json.dumps(package))
